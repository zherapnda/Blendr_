/**
 * Seed script to populate the database with sample users and groups
 * 
 * Usage:
...
async function seedData() {
  ...
  try {
    ...
    for (const profile of sampleProfiles) {
      // Generate a random password for each user (they won't need to login)
      const tempPassword = `TempPass${randomUUID().substring(0, 8)}!`;
    ...
    console.log(`\n✅ Created ${createdUserIds.length} users and profiles\n`);
    ...
    console.log(`\n✅ Created ${createdGroupIds.length} groups\n`);
    ...
    console.log('\n📊 Summary:');
    console.log(`   - ${createdUserIds.length} users/profiles created`);
    console.log(`   - ${createdGroupIds.length} groups created`);
    console.log(`   - ${membershipsAdded} group memberships added`);
    console.log('\n💡 Note: The 2 CS students (Alex Chen and Jordan Martinez) are optimized for hackathon searches!');
